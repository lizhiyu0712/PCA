...
for task = 1:numel(class)
    ...
    % select the axis (X=2, Y=3, Z=4)
    signal = table2array(raw_data(selected_task, 3));      
...
xlabel('time', 'fontsize', 12);
sgtitle('Accelerometer Y-axis', 'fontsize', 12);
...
for task = 1:numel(class)
    ...
    % select the axis (X=2, Y=3, Z=4)
    signal = table2array(raw_data(selected_task, 4));      
...
xlabel('time', 'fontsize', 12);
sgtitle('Accelerometer Z-axis', 'fontsize', 12);
