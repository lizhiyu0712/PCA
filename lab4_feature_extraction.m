...
for task = 1:numel(class)
    ...
    for cal_k = 1:size(unique_group, 1)
        ...
        max_feature      = feat1(data);   % calculate max of each segmented signal
        min_feature      = feat2(data);   % calculate min of each segmented signal
        mean_feature     = feat3(data);   % calculate mean of each segmented signal
        std_feature      = feat4(data);   % calculate std of each segmented signal
        skewness_feature = feat5(data);   % calculate skewness of each segmented signal
